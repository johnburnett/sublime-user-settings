# bfdpylinter.py, wrapper for bfdpylint for SublimeLinter3 Sublime Text plugin
# Ripped off from SublimeLinter-pylint

import re
from SublimeLinter.lint import Linter, util, persist


class Bfdpylint(Linter):
    ...
    regex = (
        # Example error we are parsing:
        # "E1004 (missing-super-argument):287: Missing argument to super()"

        # Regexes for individual chunks of above error:
        # "E1004 "
        r'^(?P<code>(?:(?P<error>[FE])|(?P<warning>[CIWR]))\d+) '
        #       "(missing-super-argument):287: "
        r'(?P<symbolic_name>\([a-zA-Z0-9\-]+\)):(?P<line>\d*): '
    ...
    defaults = {
        # allows the user to say whether they want to see the error code
        'show-codes': True,
    }
    ...
    def split_match(self, match):
        ...
        """
        match, line, col, error, warning, message, near = super(Bfdpylint, self).split_match(match)
